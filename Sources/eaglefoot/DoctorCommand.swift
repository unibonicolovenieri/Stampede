...
struct Doctor: AsyncParsableCommand {
    ...
    private func runBenchmark() async throws {
        ...
        let workspace = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("eaglefoot-bench-\(UUID().uuidString.prefix(8))")
...
enum SampleGenerator {
    ...
    static func generateLogo(into directory: URL) throws -> URL {
        ...
        else { throw EagleFootError.invalidConfiguration("impossibile creare il logo di prova") }
        ...
        guard let cgImage = bitmap.makeImage() else {
            throw EagleFootError.invalidConfiguration("impossibile creare il logo di prova")
        }
