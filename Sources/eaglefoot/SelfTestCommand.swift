...
struct SelfTest: AsyncParsableCommand {
    ...
    private func checkRoundTrip(_ c: inout Checker) throws {
        ...
        let workspace = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("eaglefoot-selftest-\(UUID().uuidString.prefix(8))")
        ...
        c.assert("nessun file temporaneo lasciato in giro",
                 (try? FileManager.default.contentsOfDirectory(atPath: workspace.path))?
                     .contains(where: { $0.hasSuffix(".eaglefoot-part") }) == false)
        ...
        c.assert("EXIF preservato: modello di fotocamera",
                 (properties[kCGImagePropertyTIFFDictionary] as? [CFString: Any])?[kCGImagePropertyTIFFModel] as? String == "EagleFoot Test Cam")
    ...
    private func makePhotoWithMetadata(size: CGSize, into directory: URL) throws -> URL {
        ...
        else { throw EagleFootError.renderFailed(url) }

        guard let destination = CGImageDestinationCreateWithURL(
            url as CFURL, UTType.jpeg.identifier as CFString, 1, nil)
        else { throw EagleFootError.cannotCreateDestination(url) }

        let properties: [CFString: Any] = [
            kCGImageDestinationLossyCompressionQuality: 0.98,
            kCGImagePropertyTIFFDictionary: [
                kCGImagePropertyTIFFMake: "Lumika",
                kCGImagePropertyTIFFModel: "EagleFoot Test Cam",
            ] as [CFString: Any],
        ...
        guard CGImageDestinationFinalize(destination) else { throw EagleFootError.encodingFailed(url) }
        return url
    }
