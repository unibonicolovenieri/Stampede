...
public final class RcloneUploader: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "it.lumika.eaglefoot.upload",
    ...
    public init(target: RemoteTarget,
        ...
        guard let executable = Self.locateExecutable() else { throw EagleFootError.rcloneMissing }
    ...
    public func copyAll(transfers: Int = 8, showProgress: Bool = true) throws -> Int32 {
        ...
        if result.exitCode != 0 {
            throw EagleFootError.rcloneFailed(exitCode: result.exitCode, stderr: result.stderr)
        }
