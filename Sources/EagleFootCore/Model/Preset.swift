...
public struct PresetStore: Sendable {
    /// `~/.config/eaglefoot/presets` — dove finiscono i preset salvati dall'utente.
    public static var userDirectory: URL {
        if let xdg = ProcessInfo.processInfo.environment["XDG_CONFIG_HOME"], !xdg.isEmpty {
            return URL(fileURLWithPath: xdg).appendingPathComponent("eaglefoot/presets", isDirectory: true)
        }
        return FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/eaglefoot/presets", isDirectory: true)
    }
    ...
    public func load(_ nameOrPath: String) throws -> Preset {
        ...
        throw EagleFootError.presetNotFound(nameOrPath)
    }
    ...
    private func decode(at url: URL) throws -> Preset {
        ...
        do {
            ...
            throw EagleFootError.invalidConfiguration("preset illeggibile \(url.lastPathComponent): \(error.localizedDescription)")
        }
    ...
    public func delete(_ name: String) throws {
        ...
        throw EagleFootError.presetNotFound(name)
    }
