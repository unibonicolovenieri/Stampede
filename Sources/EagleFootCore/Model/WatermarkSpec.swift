...
public struct WatermarkSpec: Codable, Sendable, Equatable {
    ...
    public func validated() throws -> WatermarkSpec {
        ...
        guard (0...1).contains(opacity) else {
            throw EagleFootError.invalidConfiguration("opacity deve essere fra 0 e 1 (valore: \(opacity))")
        }
        guard scale.value > 0 else {
            throw EagleFootError.invalidConfiguration("la scala del logo deve essere > 0")
        }
        if layout == .tile {
            guard tile.spacingX > -0.99, tile.spacingY > -0.99 else {
                throw EagleFootError.invalidConfiguration("spacing del mosaico troppo negativo")
            }
        }
        return self
    }
