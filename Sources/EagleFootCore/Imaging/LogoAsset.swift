...
public final class LogoAsset: @unchecked Sendable {
    ...
    public static func load(_ url: URL) throws -> LogoAsset {
        ...
        guard FileManager.default.fileExists(atPath: url.path) else {
            throw EagleFootError.cannotDecodeLogo(url)
        }
        switch ext {
        ...
        default:
            throw EagleFootError.unsupportedLogoFormat(ext)
        }
    }
    ...
    private init(raster url: URL) throws {
        ...
        else { throw EagleFootError.cannotDecodeLogo(url) }
        self.url = url
        self.kind = .raster
    ...
    private init(pdf url: URL) throws {
        ...
        guard box.width > 0, box.height > 0 else { throw EagleFootError.cannotDecodeLogo(url) }
        self.url = url
        self.kind = .vector
        self.baseImage = nil
    ...
    public func image(atWidth width: CGFloat) throws -> CIImage {
        ...
        guard let baseImage else { throw EagleFootError.cannotDecodeLogo(url) }
