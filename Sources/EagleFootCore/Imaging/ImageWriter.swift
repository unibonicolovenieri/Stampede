...
public enum ImageWriter {
    ...
    public static func write(_ image: CIImage,
        ...
        guard extent.width >= 1, extent.height >= 1 else {
            throw EagleFootError.renderFailed(source.url)
        }
        ...
        guard let cgImage = context.createCGImage(image,
                                                  ...
                                                  deferred: false)
        else { throw EagleFootError.renderFailed(source.url) }

        try FileManager.default.createDirectory(at: url.deletingLastPathComponent(),
                                                withIntermediateDirectories: true)

        let temporary = url.deletingLastPathComponent()
            .appendingPathComponent(".\(url.lastPathComponent).eaglefoot-part")
        ...
        guard CGImageDestinationFinalize(destination) else {
            throw EagleFootError.encodingFailed(url)
        }
